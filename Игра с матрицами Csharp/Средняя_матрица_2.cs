...
{
    public partial class Средняя_матрица_2 : Form
    {
        float[,] matrix_result = new float[5, 5];
        int rnd_i_1, rnd_j_2;

        public Средняя_матрица_2()
        {
            InitializeComponent();
        }

        private void Средняя_матрица_2_Load(object sender, EventArgs e)
        {
            float[,] matrix_1 = new float[5, 5], matrix_2 = new float[5, 5];
            int i, j, cells_addition_cycle, rnd_j_1, rnd_i_2;

            Random rnd = new Random();
            rnd_j_1 = rnd.Next(3, 5);
            rnd_i_2 = rnd.Next(3, 5);
            do //генерация чисел для ранга матрицы
            {
                rnd_i_1 = rnd.Next(3, 5);
                rnd_j_2 = rnd.Next(3, 5);
